...
def usage():
    ...
    print "[-u/--unmapped] <report XM:i:2 from mapped and generated BAM in a supplementary BAM file>"
    print "[-h/--help] <Help>"
    return
...
if __name__ == "__main__":
    ...
    if (rep_xm2):
        gen_xm2 = pysam.AlignmentFile(output_dir + REPBAM, "wb", template=filegenbam)
    ...
    for x,y in itertools.izip(filemapbam.fetch(until_eof=True),filegenbam.fetch(until_eof=True)):
        ...
        if (x.pos==y.pos):                            # Read mapped correctly (consider that read mapping to the exact same region on another chromosome is pretty unlikely)
            ...
            if (rep_xm2):
                #gen_xm2.write(x)
                y.flag = x.flag               # Write the flag of the mapped read
                gen_xm2.write(y)
        outfile.write(x)
