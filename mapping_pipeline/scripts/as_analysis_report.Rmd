...
R --no-save --no-restore -e "outPath='./'; inPath='/data/kdi_prod/.kdi/project_workspace_0/650/acl/04.00/rna_analysis/results/ascount'; min_gene_cov='10'; gene.in='/data/kdi_prod/.kdi/project_workspace_0/650/acl/04.00/rna_analysis/annotations/Mus_musculus.NCBIM37.cleaned.gtf'; known.escapees='./data/escapees_Berletch.txt'; rmarkdown::render('as_analysis_report.Rmd', output_file='${ODIR}as_analysis_report.html')"
...
gtf.gr <- import(gene.in)
gtf.gr <- sortSeqlevels(geneannot)
gtf.gr <- sort(geneannot)

txdb <- makeTxDbFromGFF(gene.in, format="gtf")
geneannot <- genes(txdb)
geneannot$gene_name <- gtf.gr$gene_name[match(geneannot$gene_id, gtf.gr$gene_id)]
geneannot.perchr <- split(geneannot, seqnames(geneannot))
xgenes <- unique(geneannot[grep("X", seqnames(geneannot))]$"gene_name")
autogenes <- unique(geneannot[grep("X|Y|M",seqnames(geneannot), invert=TRUE)]$"gene_name")
...
##
exprs.in <- list.files(path=inPath, pattern="asratio.tsv$", full.names=TRUE, recursive=TRUE)
...
allst <- lapply(1:length(das), function(i){
  x  <- das[[i]]
  xa <- x[intersect(rownames(x), autogenes),]
  p <- ggplot(as.data.frame(xa), aes(x=log2(genome1+1), y=log2(genome2+1))) + geom_point() + xlab("Genome 1") + ylab("Genome 2") +
      geom_abline(intercept=1, linetype="dashed") + annotate("text",label = "Autosomes", x=1, y = log2(max(xa$genome1)),  size = 4, colour = "darkgray")+
          theme(axis.title = element_text(face="bold")) + ggtitle(names(das)[i])
  return(p)
})
png(file.path(outPath, "scatter_ascounts_auto.png"), width=10, height=15, res=300, units="in")
do.call("grid.arrange", c(allst, ncol=floor(sqrt(length(allst)))))
dev.off()

allst.X <- lapply(1:length(das), function(i){
    x  <- das[[i]]
    xx <- x[intersect(rownames(x), xgenes),]
    
    p <- ggplot(as.data.frame(xx), aes(x=log2(genome1+1), y=log2(genome2+1))) + geom_point() + xlab("Genome 1") + ylab("Genome 2") +
        geom_abline(intercept=1, linetype="dashed") + annotate("text", label = "X chromosome", x=2, y = log2(max(xa$genome1)+1),  size = 4, colour = "darkgray") +
            theme(axis.title = element_text(face="bold")) + ggtitle(names(das)[i])
    return(p)
})

#png(file.path(outPath, "scatter_ascounts_onX.png"), width=10, height=15, res=300, units="in")
do.call("grid.arrange", c(allst.X, ncol=floor(sqrt(length(allst.X)))))
#dev.off()
...
allbx <- lapply(1:length(das), function(i){
  ...
  p <- ggplot(d, aes(x = grp, y = x)) + geom_boxplot() + geom_hline(yintercept=0.5, linetype="dashed") +
    xlab("") + ylab("AS ratio") + theme(axis.title = element_text(face="bold")) + ggtitle(names(das)[i])
  return(p)
})

#png(file.path(outPath, "boxplot_asratio.png"), width=15, height=15, res=300, units="in")
do.call("grid.arrange", c(allbx, ncol=floor(sqrt(length(allbx)))))
#dev.off()
...
asHeatmap(x=das, genes.list=allXesc, show.values=TRUE, intersect=FALSE)
#dev.off()
```
