...
ASspecgeno1="/bioinfo/users/khillion/AS_proj/generated_reads/data/beds/CASTspec.bed"
ASspecgeno2="/bioinfo/users/khillion/AS_proj/generated_reads/data/beds/129spec.bed"
...
then
	...
	${simreads}scripts/build_parental_reference.sh -v ${vcf_geno1} -i ${id_geno1} -c ${config}
fi
if [[ ! -e ${fasta_outdir}${id_geno2}.fa ]]
then
	echo "Generation of the parental chromosome for ${id_geno2} ..."
	${simreads}scripts/build_parental_reference.sh -v ${vcf_geno2} -i ${id_geno2} -c ${config}
fi
...
# AS geno1
${simreads}scripts/generate_reads.sh -i ${id_geno1} -b ${bed_outdir}${id_geno1}spec_${chr}_${int_bed} -n ${number_reads} -c ${config}
${simreads}scripts/generate_reads.sh -i ${id_geno2} -b ${bed_outdir}${id_geno1}spec_${chr}_${int_bed} -n $((number_reads/10)) -c ${config}
# AS geno2
${simreads}scripts/generate_reads.sh -i ${id_geno1} -b ${bed_outdir}${id_geno2}spec_${chr}_${int_bed} -n $((number_reads/10)) -c ${config}
${simreads}scripts/generate_reads.sh -i ${id_geno2} -b ${bed_outdir}${id_geno2}spec_${chr}_${int_bed} -n ${number_reads} -c ${config}
# no AS
${simreads}scripts/generate_reads.sh -i ${id_geno1} -b ${bed_outdir}other_${chr}_${int_bed} -n $((number_reads/2)) -c ${config}
${simreads}scripts/generate_reads.sh -i ${id_geno2} -b ${bed_outdir}other_${chr}_${int_bed} -n $((number_reads/2)) -c ${config}
