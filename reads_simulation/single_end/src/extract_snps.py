...
If no --ref is specified, use the REF allele as is. The VCF contains all the homozygous SNPs for the --alt strain.
Otherwise, replace the REF allele by the specified one. The VCF format corresponds to the F1 of the two strains. 
    ...
    for line in vcf_handle:
        ...
        if line.startswith('##'):
            ...
            if refidx != -1:
                ...
                print str('\t'.join(header[0:9])) + "\t" + altSample # Replacement of : "REF-" + altSample + "-F1"
            continue
        else:
            if altidx == -1 :
                ...
                if filt_qual != 1 or (filt_qual == 1 and reffi == str(1) and altfi == str(1)):
                    ...
                    if geno == -1:
                        ...
                        if refidx != -1:
                            ...
                            print chrom + "\t" + fields[1] + "\t" + fields[2] + "\t" + geno[0] + "\t" + geno[1] + "\t" + fields[5] + "\t" + fields[6] + "\t" + fields[7] + "\t" + "GT" + "\t" + "1/1"
                else:
                    badqual_counter+=1
            else:
                badqual_counter+=1
